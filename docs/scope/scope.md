...
_Re-skin done 2026-09-07 (spec 0005 revision, build plan steps R1 to R5, branch `feat/design-system-reskin`): neutral high-contrast `--primary`, amber moved to `--rating`, `--radius` `1rem`, Fraunces dropped for one sans family, warm poster-scrim and frosted-glass recipes, `Button` / `Badge` pills, `Poster` / `SwipeCard` larger radius + glass action bar. Component APIs and all 11 ACs unchanged. `/check verify` should run against the new look._
