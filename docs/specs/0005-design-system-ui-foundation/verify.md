# Verify: design system and UI foundation · spec 0005 · created 2026-09-07 · updated 2026-09-07 (build landed; re-skin pending)

_Steps derived from spec 0005 acceptance criteria. `/check verify` runs these; `/test` locks the durable ones._

## Re-skin pending (2026-09-07 spec revision)

Spec 0005 was revised to the modern cinematic direction after the first build. Until the re-skin pass (build plan steps R1 to R5) is re-`/develop`ed, these checks run against the earlier look. After the re-skin, verify:

- `--primary` is neutral (a near-white pill on dark, a near-black pill on light), not amber; the `Button` `default` variant and active `Badge` use it. Amber shows only on the focus ring and anywhere `--rating` is used.
- `--rating` exists in `:root` and `.dark` (and `--color-rating` in `@theme inline`); `--radius` is `1rem`; `:root` / `.dark` name sets still match (parity test).
- One font family: no Fraunces request in the network panel, `--font-display` resolves to the sans stack, headings at `text-3xl`+ are `font-semibold tracking-tight`.
- `docs/design.md` documents the warm poster scrim and frosted glass panel recipes and its contrast table is recomputed (no `--primary` 3.19:1 known-limit row; `--rating` on `--background` and `--card` present).
- `Button` / `Badge` pills are `rounded-full`; `Card` / `Poster` / `MovieCard` / `SwipeCard` use the larger radius; the `SwipeCard` action bar is a frosted glass pill bar. All keyboard, focus, announce, and reduce-motion behavior is unchanged.
...
- [ ] **AC-4** — `components.json` (from `shadcn@4`) has `rsc: true`, `tsx: true`, `iconLibrary: "lucide"`, `tailwind.cssVariables: true`, `tailwind.config: ""`, `tailwind.baseColor: "zinc"`, `style: "radix-nova"`, and the `@/` aliases (`components`, `utils`, `ui`, `lib`, `hooks`); the CLI-default `rtl` / `menuColor` / `menuAccent` / `registries` fields are present and not checked. `src/components/ui/` contains `button`, `card`, `input`, `label`, `badge`, `skeleton`. Render each in light and dark → the computed color values equal the documented token values (compare final colors, not `var()` references). The `Button` `default` variant computes to the neutral `--primary` (not amber); `Button` and `Badge` pills are `rounded-full`.
...
- [ ] **AC-10** — Tab through `Button`, `Input`, `MovieCard`, and each `SwipeCard` control → each shows a `:focus-visible` ring built from `--ring` (amber, distinct from the now-neutral `--primary`) with `ring-2 ring-offset-2 ring-offset-background`. `src/lib/contrast.test.ts` parses the token hex from `globals.css` and asserts every pair in `docs/design.md`'s contrast table meets its target in both themes: body pairs (`--foreground`/`--background`, `--card-foreground`/`--card`, `--primary-foreground`/`--primary`, `--secondary-foreground`/`--secondary`, `--accent-foreground`/`--accent`, `--muted-foreground`/`--background` and `/--card`, `--destructive`/`--background` and `/--card`) at 4.5:1; UI / large-text pairs (`--ring`/`--background` and `/--card`, `--rating`/`--background` and `/--card`, and each swipe hint `--like` / `--dislike` / `--seen` / `--skip` on `--background`) at 3:1. The old `--primary`/`--background` 3.19:1 known-limit row is gone (`--primary` is neutral now). If `--rating` in light mode misses 4.5:1 on `--background`, that is acceptable at the 3:1 target; do not style body-size link text with it without checking.
